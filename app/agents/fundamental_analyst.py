...
def analyze(company_name: str, context: dict) -> dict:
    sources = search_public_info(f"{company_name} business model revenue products")
    context_text = _build_context_text(sources)
    memory_text = build_memory_text(context.get("memory", {}))
    summary = generate_text(
        prompt=(
            f"请基于以下公开信息，生成 {company_name} 的基本面分析摘要，并用要点列出核心发现。\n"
            "要求：\n"
            "- 不要使用“好的”“作为某某分析师”等对话式开头。\n"
            "- 不要重复粘贴来源原文。\n"
            "- 不要输出 Markdown 装饰符号。\n"
            "- 每个核心发现都应尽量对应收入、利润、商业模式、竞争优势或经营风险。\n\n"
            f"{memory_text}\n\n"
            f"{context_text}"
        ),
        system_prompt="你是深研 Alpha 的 Fundamental Analyst，负责基本面分析。",
        max_tokens=500,
    )
    ...
    return structure_agent_result(
        ...
        confidence=0.8,
        sources=sources,
        risks=["商业模式与增长质量仍需结合最新财报复核。"],
        watch_items=["收入增速", "经营利润率", "核心业务订单量", "新业务亏损变化"],
    )
